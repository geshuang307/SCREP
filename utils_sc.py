...
class  TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xt_ge, xt_meth, xt_mut,  y, smile_graph, test_drug_dict):
        ...
        for i in range(data_len):
            ...
            smiles = xd[i]                 # String length
            target_ge = xt_ge[i]           # 15926
            ...
            c_size, features, edge_index = smile_graph[smiles]             # atoms:37 features:37*78 edge_index:84*2
            # make the graph ready for PyTorch Geometrics GCN algorithms:       
            GCNData = DATA.Data(x=torch.Tensor(features),                 
                                edge_index=torch.LongTensor(edge_index).transpose(1, 0),
                                y=torch.FloatTensor([labels]))             # Drug features were represented as a graph structure.
            
            # require_grad of cell-line for saliency map
            if self.saliency_map == True:
                GCNData.target_ge = torch.tensor([target_ge], dtype=torch.float, requires_grad=True)    # Gene features were only converted into tensors.
